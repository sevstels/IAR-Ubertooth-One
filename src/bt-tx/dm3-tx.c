...
void BT_DM3_tx(char channel, int length, char *data)
{
 ...
 unsigned short reg;
 */
 //ACCESS CODE (72 bits): preamble(4), sync word(64bit), and trailer(4bit)
 ...
 *//* 
 ...
 if((SyncWord[0]&0x80)==0x80) Preamble = 0xA0; //1 0 1 0 >| 1
 else Preamble = 0x50;                         //0 1 0 1 >| 0 
 
 //preparation trailer, core v4.0, page: 360
 if((SyncWord[7]&0x01)==0x01) Trailer = 0x05; //1 |> 0 1 0 1
